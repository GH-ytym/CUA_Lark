...
class LarkCliAdapter:
    ...
    def build_plan(self, intent: IntentType, payload: dict[str, Any]) -> CliCallPlan:
        """Create call plan by business domain instead of single-tool coupling."""
        domain = self.classify_business(intent)
        if domain == BusinessDomain.MESSAGE:
            return CliCallPlan(
                intent=intent,
                domain=domain,
                invocations=(
        ...
        if domain == BusinessDomain.CALENDAR:
            return CliCallPlan(
                intent=intent,
                domain=domain,
                invocations=(
        ...
        if domain == BusinessDomain.DOC_SHEET:
            operation = "doc_create" if intent == IntentType.DOC_CREATE else "sheet_write"
            tool_family = "lark-doc" if intent == IntentType.DOC_CREATE else "lark-sheets"
            return CliCallPlan(
                intent=intent,
                domain=domain,
                invocations=(
